            ...
            <div class="flex-auto p-6">
                ...
                <form role="form text-left" action="/owner/penjualan/tambah_produk <?= $id_penjualan ? '?id_trx=' . $id_penjualan : '' ?>" method="POST">
                    ...
                    <div class="mb-4">
                        ...
                        <select id="select-produk" name="id_produk" placeholder="Pilih Bahan..." autocomplete="off">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="number" name="jumlah" id="jumlah" value="1" class="text-sm focus:shadow-soft-primary-outline leading-5.6 ease-soft block w-full appearance-none rounded-lg border border-solid border-gray-300 bg-white bg-clip-padding py-2 px-3 font-normal text-gray-700 transition-all focus:border-fuchsia-300 focus:bg-white focus:text-gray-700 focus:outline-none focus:transition-shadow" placeholder="Jumlah" aria-label="Email">
                    </div>
                    <div class="mb-4">
    ...
    <?php if ($id_penjualan): ?>
        <div class="w-full max-w-full px-3 mt-0 md:flex-0 shrink-0 md:w-7/12 lg:w-5/12 xl:w-5/12!">
            <div class="relative z-0 flex flex-col min-w-0 break-words bg-white border-0 shadow-soft-xl rounded-2xl bg-clip-border">
                ...
                <div class="flex-auto p-6">
                    <?= view('components/alert') ?> <!-- panggil komponen alert -->
                    <form role="form text-left" action="/owner/pembelian/simpan" method="POST">
                                    ...
                                    <?php foreach ($penjualan_items as $item): ?>
                                        <tr>
                                            ...
                                            <td class="p-2 text-left align-middle bg-transparent border-b whitespace-nowrap shadow-transparent">
                                                <span class="text-xs font-semibold leading-tight text-slate-400"><?= $item['jumlah'] ?> x <?= $item['harga'] ?></span>
                                            </td>
                                            <td class="p-2 text-left align-middle bg-transparent border-b whitespace-nowrap shadow-transparent">
                                                <span class="text-xs font-semibold leading-tight text-slate-400"><?= countTotal($item)['subtotal'] ?></span>
                                            </td>
                                            <td class="p-2 text-left align-middle bg-transparent border-b whitespace-nowrap shadow-transparent">
                                                <span class="text-xs font-semibold leading-tight text-slate-400">-<?= $item['nilai_promo'] ?><?= $item['tipe_promo'] == 'persen' ? '%' : '' ?></span>
                                            </td>
                                            <td class="p-2 text-left align-middle bg-transparent border-b whitespace-nowrap shadow-transparent">
                                                <span class="text-xs font-semibold leading-tight text-slate-400"><?= countTotal($item)['grand_total'] ?></span>
                                            </td>
                                            <td class="p-2 align-middle bg-transparent border-b whitespace-nowrap shadow-transparent">
                                                <a href="/owner/penjualan/hapus_produk/<?= $item['id'] ?>" class="text-xs font-semibold leading-tight text-slate-400"> Hapus </a>
                                            </td>
                                        </tr>
                                    <?php endforeach ?>
                                <?php endif; ?>
                    ...
                    <form action="">
                        <div class="flex gap-4">
                            <a href="/owner/penjualan/batal/<?= $id_penjualan ?>" class="inline-block w-full px-6 py-3 mt-6 mb-2 font-bold text-center text-blue-800 uppercase align-middle transition-all bg-transparent border border-solid! border-blue-800! rounded-lg cursor-pointer active:opacity-85 hover:scale-102 hover:shadow-soft-xs leading-pro text-xs ease-soft-in tracking-tight-soft shadow-soft-md bg-150 bg-x-25 hover:border-slate-700">Batal</a>
                            <a href="/owner/penjualan/simpan/<?= $id_penjualan ?>" class="inline-block w-full px-6 py-3 mt-6 mb-2 font-bold text-center text-white uppercase align-middle transition-all bg-transparent border-0 rounded-lg cursor-pointer active:opacity-85 hover:scale-102 hover:shadow-soft-xs leading-pro text-xs ease-soft-in tracking-tight-soft shadow-soft-md bg-150 bg-x-25 bg-gradient-to-tl from-gray-900 to-slate-800 hover:border-slate-700 hover:bg-slate-700 hover:text-white">Simpan</a>
                        </div>
                    </form>
                </div>
            </div>
